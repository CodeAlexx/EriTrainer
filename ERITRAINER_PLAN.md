...
- `/home/alex/EriDiffusion/EriDiffusion-v2/crates/eridiffusion-core/src/training/progress.rs`
...
EDv2 train bins to launch: `crates/eridiffusion-cli/src/bin/train_*.rs` (17 models).
...
- **STATUS 2026-06-19:** M0 + chrome + surface + **M1 Klein launch wiring all DONE**
  ...
  cache + an EDv2 `--config` JSON). Remaining: P3 wire the other 16 models' argv into
  `build_command`; live GUI eyeball on a display.
- **M1 — Klein vertical:**
  ...
  - **Gate:** `cargo check` + `cargo test` green; window runs; launching Klein shows
    ...
    widget lies.
- **M2+ — tab fan-out + remaining models** (LoRA/Captioner/Cloud/Backup/Runs/Logs,
  then the other 16 `train_*` targets), each gated the same way.
