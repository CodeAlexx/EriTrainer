...
fn vae_log_stage(name: &str, t: &Tensor) {
    ...
    if save_on {
        ...
        let out_dir = std::env::var("ERITRAINER_VAE_STAGE_DIR")
            .map(PathBuf::from)
            .unwrap_or_else(|_| PathBuf::from("output/vae_stages"));
        let _ = std::fs::create_dir_all(&out_dir);
        let p = out_dir.join(format!("vae_stage_{idx:02}_{safe_name}.safetensors"));
